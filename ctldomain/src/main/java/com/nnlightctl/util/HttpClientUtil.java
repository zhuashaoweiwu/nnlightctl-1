...
public class HttpClientUtil {
    private  static  HttpClientUtil hu = new HttpClientUtil();
    ...
    private static final int SO_TIMEOUT = 6000;// 等待数据超时时间
    ...
    private HttpClientUtil(int conntimeout, int sotimeout) {
        this.sotimeout = sotimeout;
        this.conntimeout = conntimeout;
    }

    private HttpClientUtil(int maxConnection, int conntimeout, int sotimeout) {
        this(conntimeout, sotimeout);
        this.maxConnection = maxConnection;
    }

    private HttpClientUtil(int maxConnection, String charset, int conntimeout, int sotimeout) {
        this(conntimeout, sotimeout);
        this.maxConnection = maxConnection;
        this.reqCharset = charset;
    }

    private HttpClientUtil(int maxConnection) {
        this.maxConnection = maxConnection;
    }

    private HttpClientUtil(int maxConnection, String charset) {
        this.maxConnection = maxConnection;
        this.reqCharset = charset;
    }

    private HttpClientUtil(int maxConnection, String charset, String resCharset) {
        this.maxConnection = maxConnection;
        this.reqCharset = charset;
    ...
    public String postData(String url, final HttpEntity entity, ContentType type) throws IOException {
    ...
    public String postData(String url, final List<NameValuePair> params, ContentType type) throws IOException {
        return fetchData(this.createPost(url, params, type));
    }

    public String postBodyDataWithJSON(String url, String json) throws IOException {
        return fetchData(this.createPost(url, new StringEntity(json), ContentType.APPLICATION_JSON));
    }
    ...
    public String fetchData(HttpRequestBase request) {
        ...
        try {
            ...
            if (response.getStatusLine()
                ...
                Charset rescharset = ContentType.getOrDefault(rsentity)
                        .getCharset();
                if (rescharset != null && rescharset.name()
                        .equals("ISO-8859-1")) {
                ...
                logger.error("fetch request return error status:{} code:{}", request.getURI(), response.getStatusLine()
                        .getStatusCode());
            }
     ...
     */
    public HttpPost createPost(String url, final List<NameValuePair> params, ContentType type)
            throws UnsupportedEncodingException {
        HttpEntity entity = new UrlEncodedFormEntity(params, reqCharset);
    ...
    public HttpPost createPost(String url, HttpEntity entity, ContentType accept) {
        HttpPost method = new HttpPost(url);
        if (null != accept) {
            method.addHeader("Content-type", accept.toString());
        }
